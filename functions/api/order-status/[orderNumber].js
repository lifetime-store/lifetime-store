...
export async function onRequestGet(context) {
  ...
  if (isTracking) {
    const sql = `SELECT d.*, o.id as order_id, o.order_number, o.customer_name, o.email, o.status as order_status, o.total, o.currency, o.created_at, o.country, o.city
    ...
    order = await context.env.DB.prepare(`SELECT id, order_number, customer_name, email, status, total, currency, created_at, promo_code, promo_discount, tier_discount, country, city FROM orders WHERE order_number = ? AND lower(email) = lower(?) LIMIT 1`).bind(identifier, email).first();
  ...
  return ok({ order, history: hist.results || [], delivery, deliveryHistory: deliveryHistory.results || [], deliveryFees: fees.results || [] });
}
